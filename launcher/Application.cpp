    ...
    // check update locks
    {
        ...
        if (update_lock.exists()) {
            ...
            auto infoMsg = tr("This installation has a update lock file present at: %1\n"
                              ...
                              "proceeding.\n"
                              "Check the XylarJava updater log at: \n"
                              "%7\n"
...
namespace {
const QStringList authSettingsPaths(const QString& rootPath, const QString& dataPath)
{
    return {
        FS::PathCombine(QCoreApplication::applicationDirPath(), "auth-settings.ini"),
        FS::PathCombine(rootPath, "auth-settings.ini"),
        FS::PathCombine(dataPath, "auth-settings.ini"),
    };
}

std::unique_ptr<QSettings> readAuthSettings(const QString& path)
{
...
QString readMSAClientIDFromSettings(QSettings& settings)
{
    const QString sectionValue = settings.value("Auth/MSAClientID").toString().trimmed();
    if (!sectionValue.isEmpty()) {
        return sectionValue;
    }
    return settings.value("MSAClientID").toString().trimmed();
}

bool readMSALauncherEndpointFromSettings(QSettings& settings)
{
    return settings.value("Auth/MSAUseLauncherEndpoint", settings.value("MSAUseLauncherEndpoint", false)).toBool();
}
}  // namespace
...
{
    if (m_settings) {
        const QString override = m_settings->get("MSAClientIDOverride").toString().trimmed();
        if (!override.isEmpty()) {
            return override;
        }
    }

    for (const QString& path : authSettingsPaths(m_rootPath, m_dataPath)) {
        auto settings = readAuthSettings(path);
        if (!settings) {
            continue;
        }
        const QString clientId = readMSAClientIDFromSettings(*settings);
        if (!clientId.isEmpty()) {
            return clientId;
        }
    }
...
bool Application::useMSALauncherLoginEndpoint()
{
    for (const QString& path : authSettingsPaths(m_rootPath, m_dataPath)) {
        auto settings = readAuthSettings(path);
        if (settings && readMSALauncherEndpointFromSettings(*settings)) {
            return true;
        }
    }
    return false;

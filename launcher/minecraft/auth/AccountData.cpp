...
void tokenToJSONV3(QJsonObject& parent, const Token& t, const char* tokenName)
{
    ...
    bool save = false;
    if (!t.token.isEmpty()) {
        out["token"] = QJsonValue(AccountTokenCipher::encrypt(t.token));
        save = true;
    }
    if (!t.refresh_token.isEmpty()) {
        out["refresh_token"] = QJsonValue(AccountTokenCipher::encrypt(t.refresh_token));
        save = true;
    }
    if (t.extra.size()) {
...
Token tokenFromJSONV3(const QJsonObject& parent, const char* tokenName)
{
    ...
    if (token.isString()) {
        out.token = AccountTokenCipher::decrypt(token.toString());
        out.validity = Validity::Assumed;
    }

    auto refresh_token = tokenObject.value("refresh_token");
    if (refresh_token.isString()) {
        out.refresh_token = AccountTokenCipher::decrypt(refresh_token.toString());
    }
